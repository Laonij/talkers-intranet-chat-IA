...
function buildTeacherIdentity(rawValue = "", extraSpecialties = []) {
  const parsed = parseTeacherHeader(rawValue) || parseTeacherHeader(`TEACHER ${rawValue}`);
  if (parsed) {
    return {
      ...parsed,
  ...
  const displayName = toTitleCase(
    normalizeWhitespace(repairMojibakeText(rawValue || "").replace(TEACHER_PREFIX_RE, " ").replace(/\([^)]*\)/g, " "))
  );
  const normalizedName = TEACHER_ALIAS_MAP.get(normalizeAcademicText(displayName)) || normalizeAcademicText(displayName);
  return {
    raw_header: normalizeWhitespace(repairMojibakeText(rawValue || "")),
    display_name: displayName,
    normalized_name: normalizedName,
    aliases: Array.from(new Set([displayName, normalizeWhitespace(repairMojibakeText(rawValue || ""))].filter(Boolean))),
...
function parseVipSheet(workbook, sheetName = "VIPS", workbookName = "") {
  ...
  for (let rowIndex = 1; rowIndex < rows.length; rowIndex += 1) {
    ...
    const block = buildSyntheticClassBlock(record);
