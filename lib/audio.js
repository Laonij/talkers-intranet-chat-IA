...
const audioLogger = createLogger("uploads");
  ...
  for (let attempt = 0; attempt <= retryCount; attempt += 1) {
    try {
      ...
      if (attempt < retryCount) {
        audioLogger.warn("Falha em requisicao externa de audio; tentando novamente.", details);
        continue;
      }
...
async function transcribeAudio(filePath, filename = "", mimeType = "", options = {}) {
  ...
  if (fs.existsSync(cachePath)) {
    const cached = sanitizeTextForPostgres(fs.readFileSync(cachePath, "utf8"), {
      trim: true,
  ...
  const text = sanitizeTextForPostgres(data?.text || "", {
    trim: true,
    normalizeWhitespace: false,
  }) || "";
  if (text) {
