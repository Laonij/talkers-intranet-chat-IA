...
const INTENT_RULES = [
  ...
  { key: "document_generation", terms: /\b(documento|comunicado|relat[oó]rio|ata|proposta|apresenta[cç][aã]o|planilha|arquivo|pdf|docx|xlsx)\b/i },
...
function detectBusinessArea(text = "", departments = []) {
  const normalized = normalizeBusinessText(text);
  const explicit = BUSINESS_AREAS.find((item) => item.terms.test(normalized));
  ...
  return BUSINESS_AREAS.find((item) => item.key === "administrative");
}
...
- Resposta atual deve considerar esse contexto, evitando parecer um assistente generico.
...
function buildBusinessInstructions(businessIntent) {
  const lines = [
    "- Interprete a pergunta como parte de uma operacao real de escola de idiomas.",
...
function analyzeBusinessIntent(text = "", options = {}) {
  ...
  const area = detectBusinessArea(text, departments);
  const intentType = detectIntentType(text);
  const responseDepth = detectPreferredDepth(text, intentType);
  const businessAreaKey = area?.key || "administrative";

  return {
    intentType,
    intentTypeLabel: intentType.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()),
    businessAreaKey,
    businessAreaLabel: area?.label || "Administrativo",
    responseDepth,
    preferInternalKnowledge: businessAreaKey !== "market_research",
