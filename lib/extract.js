...
async function extractZipImageText(zip, folderPrefix) {
  ...
  for (const imageName of imageNames) {
    try {
      ...
      console.error("Erro lendo imagem interna:", imageName, err?.message || err);
    }
  }
...
async function extractPdfOcrText(filePath) {
  ...
  try {
    ...
    console.error("Erro fazendo OCR do PDF:", err?.message || err);
    return "";
  } finally {
    try {
...
function extractPlainText(filePath) {
  try {
    ...
    console.error("Erro lendo TXT/CSV/MD:", err);
    return "";
  }
}
