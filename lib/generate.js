...
function looksLikeExplicitArtifactRequest(text = "", kind = "") {
  ...
  if (kind === 'xlsx') return /(planilha|excel|xlsx|tabela)/.test(text);
  if (kind === 'pdf') return /(pdf|relatorio em pdf)/.test(text);
  if (kind === 'docx') return /(docx|word|documento)/.test(text);
...
function detectArtifactKind(prompt, options = {}) {
  ...
  if (/(imagem|foto|ilustracao|logo|banner|cartaz|poster|capa|arte|mockup|avatar|retrato)/.test(text)) {
...
async function callResponsesText(apiKey, prompt, targetKind) {
  ...
  const instructionsByKind = {
    ...
    docx: "Voce esta redigindo o conteudo final de um documento profissional em portugues do Brasil, pronto para ser enviado. Entregue um texto polido, com titulo, subtitulo, secoes claras, tom comercial ou institucional conforme o caso e fechamento apropriado. Nao explique o que voce vai fazer. Nao repita literalmente o pedido do usuario. Nao use markdown. Nao use placeholders entre colchetes. Sempre produza conteudo final.",
    code: "Responda apenas com o codigo solicitado, sem cercas markdown.",
    xlsx: "Descreva em texto simples e estruturado quais abas, colunas, formulas e registros iniciais uma planilha final deve ter. Seja concreto e pratico.",
