...
function detectArtifactKind(prompt) {
  ...
  if (/(imagem|foto|ilustracao|ilustração|logo|banner|cartaz|poster|capa)/.test(text)) {
    return "image";
  }

  if (/(planilha|excel|xlsx)/.test(text)) return "xlsx";
  if (/(pdf|relatorio em pdf|relatório em pdf)/.test(text)) return "pdf";
