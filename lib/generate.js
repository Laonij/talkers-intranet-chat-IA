...
function detectArtifactKind(prompt, options = {}) {
  ...
  if (/(imagem|foto|ilustracao|ilustracao|logo|banner|cartaz|poster|capa|arte|mockup)/.test(text)) {
    return "image";
  }
...
async function createImageGenerationArtifact(apiKey, prompt, outDir) {
  ...
  return saveBase64ImageArtifact(prompt, outDir, imageBase64, "Imagem gerada com sucesso.");
}
...
async function createImageEditArtifact(apiKey, prompt, outDir, referenceImages = []) {
  ...
  for (const reference of eligibleImages.slice(0, 1)) {
  ...
  const resp = await fetch(IMAGE_EDITS_URL, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${apiKey}`,
    },
  ...
  return saveBase64ImageArtifact(prompt, outDir, imageBase64, "Imagem editada com sucesso.");
}
