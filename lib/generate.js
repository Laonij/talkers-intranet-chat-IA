...
function isMetaQuestionAboutCreatingImage(text = "") {
  return /(por que|porque|como|quando|se|voce pode|você pode|tu pode|tu consegue|voce consegue|você consegue|nao pode|não pode|nao consegue|não consegue|qual a limitacao|qual a limitação|quais as limitacoes|quais as limitações|por que nao|por que não)/.test(text);
}

function wantsImageGeneration(text = "") {
  const hasImageNoun = /(imagem|foto|ilustracao|ilustração|logo|banner|cartaz|poster|capa|arte|mockup|thumbnail|flyer)/.test(text);
  const hasCreationVerb = /(gere|gerar|gera\b|crie|criar|cria\b|fa[cç]a|fazer|monte|montar|produza|produzir|desenvolva|desenvolver|renderize|renderizar)/.test(text);
  const isDirectCreativeAsk = /(^|\b)(quero|preciso|gostaria|faz|fa[cç]a|crie|gere)\b/.test(text);
  const hasDownloadCue = /(para eu baixar|pra eu baixar|para baixar|download|baixar)/.test(text);
  const metaQuestion = isMetaQuestionAboutCreatingImage(text);

  if (metaQuestion) return false;
  if (hasImageNoun && (hasCreationVerb || hasDownloadCue || isDirectCreativeAsk)) return true;
  if (/(banner|cartaz|poster|logo|thumbnail|flyer)/.test(text) && isDirectCreativeAsk) return true;
  return false;
}
