...
function hasQuestionIntent(text = "") {
  return /(^|\s)(quem|o que|qual|quais|como|porque|por que|voce consegue|você consegue|voces conseguem|vocês conseguem|da para|dá para|pode me explicar|pode explicar)(\s|$)|\?/.test(text);
}

function isMetaCapabilityQuestion(text = "") {
  return /(voce consegue|você consegue|voces conseguem|vocês conseguem|por que nao|por que não|porque nao|porque não|da para|dá para|e possivel|é possível|consegue mesmo|voce pode|você pode).{0,40}(gerar|criar|fazer|produzir).{0,20}(imagem|arte|banner|cartaz|logo|foto)|(?:gerar|criar|fazer|produzir).{0,20}(imagem|arte|banner|cartaz|logo|foto).{0,40}(voce consegue|você consegue|por que nao|por que não|porque nao|porque não|da para|dá para|e possivel|é possível)/.test(text);
}

function hasMarketingVisualIntent(text = "") {
  return /(campanha|marketing|instagram|rede social|redes sociais|social media|post|story|stories|feed|anuncio|anúncio|divulgar|criativo)/.test(text) && /(imagem|arte|banner|cartaz|poster|capa|logo|mockup|design|visual|criativo)/.test(text);
}

function hasRetryIntent(text = "") {
  return /^(ok|okay|sim|pode|pode gerar|gere|faça|faca|tente|tente novamente|gere novamente|faça novamente|faca novamente|repita|continue|pode seguir|siga|entao faca|então faça)/.test(text);
}
...
function detectArtifactKind(prompt, options = {}) {
  ...
  const retryIntent = hasRetryIntent(text);

  if (isMetaCapabilityQuestion(text)) {
    return null;
  }

  if (questionIntent && !generationIntent && !retryIntent) {
    return null;
  }

  if (wantsImageEdit && (generationIntent || referenceImages.length > 0 || retryIntent)) {
    return "image_edit";
  }

  if (/(imagem|foto|ilustracao|logo|banner|cartaz|poster|capa|arte|mockup|avatar|retrato)/.test(text) || hasMarketingVisualIntent(text)) {
    return (generationIntent || retryIntent) ? "image" : null;
  }

  if (/(audio|audios|voz|locucao|narracao|narração|locução|mp3|podcast)/.test(text)) {
    return (generationIntent || retryIntent) ? "audio" : null;
  }

  if (/(planilha|excel|xlsx|tabela)/.test(text)) {
    if (analysisIntent && !generationIntent) return null;
    if (/(anexo|arquivo|enviei|enviado|nessa planilha|nesta planilha|essa planilha|esta planilha)/.test(text) && analysisIntent) return null;
    return (generationIntent || retryIntent) ? "xlsx" : null;
  }
  if (/(pdf|relatorio em pdf)/.test(text)) return (generationIntent || retryIntent) ? "pdf" : null;
  if (/(docx|word|documento)/.test(text)) return (generationIntent || retryIntent) ? "docx" : null;
  if (/(codigo|script|javascript|node|html|css|sql|json|typescript|python)/.test(text)) {
    return (generationIntent || retryIntent) ? "code" : null;
  }
