...
function detectArtifactKind(prompt, options = {}) {
  ...
  const metaArtifactQuestion = hasMetaArtifactQuestion(text);
  const attachmentAnalysisIntent = hasAttachmentAnalysisIntent(text);

  if (metaArtifactQuestion) {
    return null;
  }

  if (attachmentAnalysisIntent && !generationIntent) {
    return null;
  }
  ...
  if (/(imagem|foto|ilustracao|logo|banner|cartaz|poster|capa|arte|mockup|avatar|retrato|instagram post|post para instagram|story|stories)/.test(text)) {
  ...
  if (/(pdf|relatorio em pdf|comunicado em pdf|documento em pdf)/.test(text)) return (generationIntent || retryIntent) ? "pdf" : null;
  if (/(docx|word|documento|proposta comercial|comunicado|relatorio|relatório)/.test(text)) {
    if (analysisIntent && !generationIntent) return null;
    return (generationIntent || retryIntent) ? "docx" : null;
  }
...
function buildDocumentFallback(prompt = "", kind = "pdf") {
  ...
  if (/(proposta comercial|proposta)/.test(text)) {
    return [
      "PROPOSTA COMERCIAL",
      "",
      "Apresentamos uma proposta comercial estruturada para atender à necessidade do cliente, com foco em clareza, valor percebido e facilidade de aprovação.",
      "",
      "1. Objetivo",
      "Apresentar uma solução comercial clara, profissional e alinhada à necessidade do cliente.",
      "",
      "2. Escopo da proposta",
      "- Atendimento personalizado",
      "- Execução organizada por etapas",
      "- Suporte para acompanhamento e ajustes",
      "- Entrega com acompanhamento próximo",
      "",
      "3. Diferenciais",
      "- Metodologia estruturada",
      "- Comunicação próxima com o cliente",
      "- Flexibilidade para ajustes conforme contexto",
      "",
      "4. Condições gerais",
      "Os detalhes finais de investimento, prazos e responsáveis podem ser ajustados conforme alinhamento comercial.",
      "",
      "Atenciosamente,",
      "Equipe Talkers",
    ].join("\n");
