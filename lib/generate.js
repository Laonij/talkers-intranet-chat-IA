...
const artifactLogger = createLogger("openai");
  ...
  for (let attempt = 0; attempt <= retryCount; attempt += 1) {
    try {
      ...
      if (attempt < retryCount) {
        artifactLogger.warn("Falha em geracao externa; tentando novamente.", details);
        continue;
      }
...
function hasGenerationIntent(text = "") {
  return /(\bgere\b|\bgerar\b|\bcrie\b|\bcriar\b|\bmonte\b|\bmontar\b|\bproduza\b|\bproduzir\b|\bfa[?c]a\b|\bfazer\b|\bdesenvolva\b|\bdesenvolver\b|\bconstrua\b|\bconstruir\b|\belabore\b|\belaborar\b|\bdesenhe\b|\bdesenhar\b|\btransforme\b|\btransformar\b|\bexporte\b|\bexportar\b)/.test(text);
}
...
function hasQuestionIntent(text = "") {
  return /(^|\s)(como|por que|porque|qual|quais|quando|onde|quem|voce consegue|voc? consegue|pode me dizer|me diga|me explica|me explique)(\s|\?|$)/.test(text);
}
...
async function callResponsesText(apiKey, prompt, targetKind) {
  if (!apiKey) return "";
  const safePrompt = sanitizeTextForPostgres(prompt || "", {
    trim: true,
...
function normalizeDocumentDraft(prompt = "", draftText = "", kind = "pdf") {
  const safeDraft = sanitizeTextForPostgres(draftText || "", {
    trim: true,
    normalizeWhitespace: false,
  }) || "";
...
function buildSpreadsheetRows(prompt, draftText) {
  ...
  if (/(lead|leads|crm|prospect|comercial|captacao|captacao de leads)/.test(text)) {
    return [
      ["Nome", "Empresa", "Telefone", "Email", "Origem", "Responsavel", "Etapa", "Ultimo contato", "Proximo passo", "Observacoes"],
      ["", "", "", "", "Instagram", "", "Novo lead", "", "", ""],
    ];
  }

  if (/(escala|agenda|plantao|plantoes|horario|horarios|turno|equipe)/.test(text)) {
    return [
      ["Data", "Dia da semana", "Turno", "Responsavel", "Area", "Inicio", "Fim", "Status", "Observacoes"],
      ["", "", "Manha", "", "", "08:00", "12:00", "Planejado", ""],
    ];
  }
  ...
  if (/(financeiro|despesa|receita|pagamento|caixa)/.test(text)) {
    return [
      ["Data", "Descricao", "Categoria", "Centro de custo", "Valor previsto", "Valor realizado", "Status", "Observacoes"],
      ["", "", "", "", "", "", "Pendente", ""],
    ];
  }

  if (/(fornecedor|fornecedores|parceiro|parceiros)/.test(text)) {
    return [
      ["Fornecedor", "Categoria", "Contato", "Telefone", "Email", "Cidade", "Status", "Observacoes"],
      ["", "", "", "", "", "", "Ativo", ""],
    ];
  }

  const rows = [
    ["Item", "Descricao", "Responsavel", "Status", "Data", "Observacoes"],
    ["1", String(prompt || "").trim() || "Planilha automatica", "", "Aberto", new Date().toISOString().slice(0, 10), ""],
  ];
