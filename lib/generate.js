...
function detectArtifactKind(prompt = "", options = {}) {
  ...
  if (/(audio|narre|narracao|voz|tts|locucao|locu????o)/.test(value)) return "audio";
  if (/(pptx|powerpoint|slides|slide|apresentacao|apresenta????o|deck)/.test(value)) return "pptx";
  if (/(planilha|xlsx|excel|csv|aba de resumo|kpi|kpis)/.test(value)) return "xlsx";
  if (/(pdf)/.test(value)) return "pdf";
  if (/(docx|word|documento|relatorio|relat??rio|proposta|contrato|comunicado|ata|manual|checklist|resumo executivo|carta)/.test(value)) return "docx";

  const visualSubjectCue = /\b(imagem|foto|retrato|selfie|avatar|banner|arte|capa|ilustracao|ilustra????o|poster|thumbnail|logo|visual|aparencia|rosto|fundo|cenario|roupa)\b/.test(value);
  const attachmentReferenceCue = /\b(me|minha|minha foto|minha imagem|minha selfie|essa imagem|essa foto|esse retrato|essa selfie|isso|isso aqui|aquilo|imagem enviada|foto enviada)\b/.test(value);
  const imageActionCue = /\b(gere|gerar|crie|criar|faca|fazer|monte|montar|produza|produzir|desenhe|desenhar|ilustre|ilustrar|transforme|transformar|edite|editar|ajuste|ajustar|deixe|coloque|troque|mude|recrie|reimaginar|reimagine|remova|adicione)\b/.test(value);
  const styleOrTransformationCue = /\b(estilo|versao|visual|profissional|futurista|anime|desenho|realista|formal|artistico|artistica|caricatura|cartoon|cinematografico|cinematografica|mais velho|mais nova|mais novo|roupa|escritorio|fundo|cenario|tema)\b/.test(value);
  const imageEditCue = imageActionCue && (attachmentReferenceCue || (hasReferenceImages && (styleOrTransformationCue || visualSubjectCue)));
  const imageGenCue = visualSubjectCue || (imageActionCue && /\b(criativo|campanha|anuncio|mockup|poster|banner|capa)\b/.test(value)) || imageEditCue;

  if (hasReferenceImages && imageEditCue) return "image_edit";
  if (imageGenCue) return imageEditCue ? "image_edit" : "image";
  return null;
}
...
async function createImageArtifact(apiKey, prompt, outDir, referenceImages = []) {
  ...
  const validReferenceImages = Array.isArray(referenceImages)
    ? referenceImages.filter((item) => item?.fullPath && fs.existsSync(item.fullPath))
    : [];
  const usingBaseImage = validReferenceImages.length > 0;
  const url = usingBaseImage ? OPENAI_IMAGE_EDITS_URL : OPENAI_IMAGE_GENERATIONS_URL;
  artifactLogger.info(usingBaseImage ? "image_edit_selected" : "image_generate_selected", {
    using_base_image: usingBaseImage,
    reference_images_received: Array.isArray(referenceImages) ? referenceImages.length : 0,
    prompt_preview: normalizePrompt(prompt).slice(0, 180),
  });
  if (usingBaseImage) {
    const base = validReferenceImages[0];
    artifactLogger.info("source_image_reused", {
      file_id: base.file_id || null,
      original_name: base.originalName || "",
      mime_type: base.mimeType || "",
    });
  } else {
    artifactLogger.info("no_source_image_found", {
      reference_images_received: Array.isArray(referenceImages) ? referenceImages.length : 0,
    });
  }
