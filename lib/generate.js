...
function isMetaCapabilityQuestion(text = "") {
  return /^(por que|porque|como|voce consegue|você consegue|voces conseguem|vocês conseguem|da para|dá para|e possivel|é possivel|e possível|é possível|qual a limitacao|qual a limitação|quais as limitacoes|quais as limitações|quando)\b/.test(text)
    || /(pode gerar|consegue gerar|sabe gerar|nao pode gerar|não pode gerar|capaz de gerar)/.test(text) && /\?$/.test(String(text || '').trim());
}
