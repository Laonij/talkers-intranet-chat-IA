...
const OPENAI_IMAGE_TIMEOUT_MS = Math.max(15000, Number(process.env.OPENAI_IMAGE_TIMEOUT_MS || 90000));
  ...
  for (let attempt = 0; attempt <= retryCount; attempt += 1) {
    try {
      return await fetch(url, {
        ...options,
        signal: options.signal || AbortSignal.timeout(Number(diagnostics.timeout_ms || OPENAI_GENERATE_TIMEOUT_MS)),
      });
    } catch (err) {
      lastError = err;
...
function isImageEditPrompt(text, hasReferenceImages = false) {
  const hasEditVerb = /(editar|edite|ajustar|ajuste|corrigir|corrija|melhorar|melhore|alinhar|alinhe|reposicionar|reposicione|mudar|mude|trocar|troque|alterar|altere|remover|remova|apagar|apague|recortar|recorte|redimensionar|clarear|escurecer|centralizar|desfocar|limpar|substituir|reescrever|retocar|transforme|transformar|estilizar|anime|desenhe me|desenhar-me|desenho)/.test(text);
  ...
  const hasDirectImageCue = /(imagem|foto|arte|banner|cartaz|poster|capa|logo|print|layout|design|anexo|mockup|selfie|retrato)/.test(text);
  const hasVisualCue = /(texto|fonte|fundo|cor|alinhamento|alinhado|margem|borda|recorte|nitidez|brilho|contraste|resolucao|sombra|elemento|posicao|posição|viking|anime|desenhada|desenhado|avatar|estilo)/.test(text);

  return hasDirectImageCue || (hasReferenceImages && hasVisualCue);
}

function hasGenerationIntent(text = "") {
  return /(\bgere\b|\bgerar\b|\bcrie\b|\bcriar\b|\bmonte\b|\bmontar\b|\bproduza\b|\bproduzir\b|\bfa[çc]a\b|\bfazer\b|\bdesenvolva\b|\bdesenvolver\b|\bconstrua\b|\bconstruir\b|\belabore\b|\belaborar\b|\bdesenhe\b|\bdesenhar\b|\btransforme\b|\btransformar\b|\bexporte\b|\bexportar\b|\bmonte\b|\bmonte-me\b)/.test(text);
}

function hasAnalysisIntent(text = "") {
  return /(\banalise\b|\banalisar\b|\banalise\s+essa\b|\banalise\s+esse\b|\bexplique\b|\bexplicar\b|\bresuma\b|\bresumir\b|\binterprete\b|\binterpretar\b|\bcompare\b|\bcomparar\b|\brevise\b|\brevisar\b|\bavalie\b|\bavaliar\b|\bdiagnostique\b|\bdiagnosticar\b|\bcomente\b|\bcomentario\b|\bmelhore\b|\bmelhorar\b|\botimize\b|\botimizar\b)/.test(text);
}
...
function hasQuestionIntent(text = "") {
  ...
  return /^(quem|que|o que|qual|quais|como|quando|onde|por que|porque|pode|voce consegue|você consegue|da para|dá para)/.test(normalizeText(value));
}
...
function isMetaCapabilityQuestion(text = "") {
  ...
  const startsLikeQuestion = /^(por que|porque|como|voce consegue|você consegue|voces conseguem|vocês conseguem|da para|dá para|e possivel|é possivel|e possível|é possível|qual a limitacao|qual a limitação|quais as limitacoes|quais as limitações|quando|se consegue|consegue)/.test(normalized);
  const asksCapability = /(pode gerar|consegue gerar|sabe gerar|nao pode gerar|não pode gerar|capaz de gerar|gera imagem|gerar imagem)/.test(normalized);
  return startsLikeQuestion || (asksCapability && /[?？]$/.test(value));
}

function shouldTreatAsAttachmentAnalysis(text = "") {
  if (!hasAnalysisIntent(text)) return false;
  return /(arquivo|anexo|anexada|anexado|enviei|mandei|subi|planilha|pdf|documento|foto|imagem|essa|esse|isso|dessa|desse|deste|desta)/.test(text);
}

function looksLikeExplicitArtifactRequest(text = "", kind = "") {
  if (!hasGenerationIntent(text)) return false;
  if (kind === 'image') return /(imagem|foto|ilustracao|logo|banner|cartaz|poster|capa|arte|mockup|avatar|retrato)/.test(text);
  if (kind === 'audio') return /(audio|audios|voz|locucao|narracao|narração|locução|mp3|podcast)/.test(text);
  if (kind === 'xlsx') return /(planilha|excel|xlsx|tabela)/.test(text) && !hasAnalysisIntent(text);
  if (kind === 'pdf') return /(pdf|relatorio em pdf)/.test(text);
  if (kind === 'docx') return /(docx|word|documento|proposta|contrato|comunicado|relatorio|relatório|oficio|ofício|declaracao|declaração)/.test(text);
  if (kind === 'code') return /(codigo|script|javascript|node|html|css|sql|json|typescript|python)/.test(text);
  return false;
}
...
function detectArtifactKind(prompt, options = {}) {
  ...
  if (isMetaCapabilityQuestion(text) && !generationIntent) return null;
  if (shouldTreatAsAttachmentAnalysis(text)) return null;
  if (hasQuestionIntent(text) && !generationIntent) return null;

  if (referenceImages.length > 0 && wantsImageEdit) {
    return "image_edit";
  }

  if (wantsImageEdit && (generationIntent || referenceImages.length > 0)) {
    return "image_edit";
  }

  if (referenceImages.length > 0 && generationIntent && /(anime|viking|desenhad|estilo|avatar|retrato|personagem|transform)/.test(text)) {
    return "image_edit";
  }

  if (/(imagem|foto|ilustracao|logo|banner|cartaz|poster|capa|arte|mockup|avatar|retrato|instagram|story|stories|reels)/.test(text)) {
    if (isMetaCapabilityQuestion(text) && !generationIntent) return null;
    return looksLikeExplicitArtifactRequest(text, 'image') ? "image" : null;
  }

  if (/(audio|audios|voz|locucao|narracao|narração|locução|mp3|podcast)/.test(text)) {
    return looksLikeExplicitArtifactRequest(text, 'audio') ? "audio" : null;
  }

  if (/(planilha|excel|xlsx|tabela)/.test(text)) {
    if (shouldTreatAsAttachmentAnalysis(text)) return null;
    return looksLikeExplicitArtifactRequest(text, 'xlsx') ? "xlsx" : null;
  }
  if (/(pdf|relatorio em pdf)/.test(text)) return looksLikeExplicitArtifactRequest(text, 'pdf') ? "pdf" : null;
  if (/(docx|word|documento)/.test(text)) return looksLikeExplicitArtifactRequest(text, 'docx') ? "docx" : null;
  if (/(codigo|script|javascript|node|html|css|sql|json|typescript|python)/.test(text)) {
    return looksLikeExplicitArtifactRequest(text, 'code') ? "code" : null;
  }
...
async function callResponsesText(apiKey, prompt, targetKind) {
  ...
  const instructionsByKind = {
    pdf: "Voce esta redigindo o conteudo final de um documento em portugues do Brasil, pronto para entregar. Entregue um documento completo, elegante e utilizavel imediatamente. Quando o pedido for comunicado, aviso, oficio, convite, proposta ou relatorio, escreva com titulo forte, abertura, contexto, secoes claras, bullets quando fizer sentido, chamada para acao ou fechamento apropriado. Nao explique o que voce vai fazer. Nao repita literalmente o pedido do usuario. Nao use markdown. Nao use placeholders entre colchetes. Sempre produza conteudo final.",
    docx: "Voce esta redigindo o conteudo final de um documento profissional em portugues do Brasil, pronto para ser enviado. Entregue um texto polido, com titulo, subtitulo, secoes claras, tom comercial ou institucional conforme o caso e fechamento apropriado. Para proposta comercial, inclua contexto, solucao, beneficios, escopo, diferenciais, investimento ou condicoes, proximos passos e fechamento. Nao explique o que voce vai fazer. Nao repita literalmente o pedido do usuario. Nao use markdown. Nao use placeholders entre colchetes. Sempre produza conteudo final.",
    code: "Responda apenas com o codigo solicitado, sem cercas markdown.",
    xlsx: "Descreva uma planilha pronta em formato tabular simples. Use linhas com colunas separadas por ponto e virgula. A primeira linha deve ser o cabecalho. Depois entregue registros iniciais concretos e uteis. Nao explique o que voce vai fazer.",
...
function normalizeDocumentDraft(prompt = "", draftText = "", kind = "pdf") {
  const promptNorm = normalizeText(prompt).replace(/\s+/g, " ").trim();
  let safeDraft = String(draftText || "").trim();
  if (!safeDraft) return buildDocumentFallback(prompt, kind);

  const lines = safeDraft.split(/\r?\n/).map((line) => line.trim());
  if (lines.length > 1) {
    const firstNorm = normalizeText(lines[0]).replace(/\s+/g, " ").trim();
    if (firstNorm && (firstNorm === promptNorm || promptNorm.includes(firstNorm) || firstNorm.includes(promptNorm))) {
      safeDraft = lines.slice(1).join("\n").trim();
    }
  }
...
function parseStructuredRowsFromDraft(draftText = "") {
  const rows = [];
...
function buildSpreadsheetRows(prompt, draftText) {
  ...
  const structuredRows = parseStructuredRowsFromDraft(draftText);
  if (structuredRows.length >= 2) {
    return structuredRows.slice(0, 80);
  }
...
async function requestGeneratedImageBase64(apiKey, prompt, size) {
  ...
  const resp = await fetchWithRetry(IMAGES_URL, {
    ...
    timeout_ms: OPENAI_IMAGE_TIMEOUT_MS,
    timeout_ms: OPENAI_IMAGE_TIMEOUT_MS,
  });
