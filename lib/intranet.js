...
function buildIntranetWorkspace({
  ...
  return {
    home: {
      ...
      stats,
      updates,
      communication_board: (announcements || []).slice(0, 6).map((item) => ({
        id: item.id,
        title: item.title,
        summary: item.summary_text || item.content_text || '',
        priority: item.priority || 'normal',
        type: item.announcement_type || 'announcement',
