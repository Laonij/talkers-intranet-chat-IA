...
function sanitizeDepartment(value = '') {
  ...
  return DEPARTMENT_LOOKUP.get(normalized) || '';
}
...
function sanitizeDepartmentList(values = []) {
  ...
  return [...new Set(
    source
      .map((item) => sanitizeDepartment(item))
      .filter(Boolean)
  )];
}
...
function buildIntranetWorkspace({ user, departments = [], recentDocuments = [], totalDocuments = 0 }) {
  const departmentAreas = departments.map((department) => buildDepartmentWorkspace(department));
  const uniqueModules = [];
