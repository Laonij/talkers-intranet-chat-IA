...
const COMMERCIAL_SHEET_INCLUDE_PATTERNS = [/^matriculas\b/, /^somente closers\b/, /^closers\b/];
const COMMERCIAL_SHEET_EXCLUDE_PATTERNS = [/^dados\b/, /^relatorio$/];
...
function isIgnoredCloserSheet(name = '') {
  ...
  return /^pagina\d+$/.test(safe)
    ...
    || COMMERCIAL_SHEET_INCLUDE_PATTERNS.some((pattern) => pattern.test(safe))
    || COMMERCIAL_SHEET_EXCLUDE_PATTERNS.some((pattern) => pattern.test(safe))
    || safe === '$$$';
}
...
function isCommercialSalesSheet(name = '') {
  const safe = normalizeSheetName(name);
  if (!safe) return false;
  if (safe === normalizeSheetName(SALES_PRIMARY_SHEET)) return true;
  if (COMMERCIAL_SHEET_EXCLUDE_PATTERNS.some((pattern) => pattern.test(safe))) return false;
  return COMMERCIAL_SHEET_INCLUDE_PATTERNS.some((pattern) => pattern.test(safe));
}

function listCommercialSalesSheetNames(workbook) {
  return listVisibleSheetNames(workbook).filter((name) => isCommercialSalesSheet(name));
}
...
function inferSalesColumnMap(header = []) {
  ...
  const phoneIndex = findColumnIndex(header, [/^telefone$/, /^fone$/, /^whatsapp$/]);
  ...
  const closerIndex = findColumnIndex(header, [/^atendente$/, /^closer$/]);
  ...
  const firstContactIndex = findColumnIndex(header, [/^primeiro contato$/]);
  const closeDateIndex = findColumnIndex(header, [/^fechamento$/]);
  const cycleIndex = findColumnIndex(header, [/^ciclo de vendas$/]);
  const observationsIndex = findColumnIndex(header, [/^observacoes$/, /^obs$/]);
  const callIndex = findColumnIndex(header, [/^call\?$/]);
  const magicClassIndex = findColumnIndex(header, [/^magic class\?$/]);
...
function deriveSaleMonth(value = '') {
  const safeDate = excelSerialToIso(value);
...
function buildImportHashes(item) {
  const dedupeBase = [
    ...
    normalizeSalesText(String(item.phone || '').replace(/\D+/g, '')),
...
function parseMatriculasWorkbook(workbook, options = {}) {
  ...
  for (let rowIndex = headerRowIndex + 1; rowIndex < rows.length; rowIndex += 1) {
    ...
    const record = {
      ...
      phone: sanitizePhoneValue(pickCell(row, map.phoneIndex)),
      ...
      observations: toVisibleString(pickCell(row, map.observationsIndex)),
      lead_stage: excelSerialToIso(pickCell(row, map.closeDateIndex)) ? 'fechado' : 'negociacao',
      source_payload: {
        header: header.map((cell) => cell.original),
        row,
        first_contact_date: excelSerialToIso(pickCell(row, map.firstContactIndex)),
        close_date: excelSerialToIso(pickCell(row, map.closeDateIndex)),
        cycle_days: toVisibleString(pickCell(row, map.cycleIndex)),
        has_call: toVisibleString(pickCell(row, map.callIndex)),
        has_magic_class: toVisibleString(pickCell(row, map.magicClassIndex)),
      },
    };

    record.sale_date = record.source_payload.close_date || record.sale_date || excelSerialToIso(pickCell(row, map.firstContactIndex));
    record.sale_month = record.sale_month || deriveSaleMonth(record.sale_date);
