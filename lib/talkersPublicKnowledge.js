...
function buildTalkersDirectAnswer(selectedSources = [], userLanguage = "pt") {
  ...
  const institutional = selected.find((item) => item.category === "institutional") || selected[0];
  const courses = selected.find((item) => item.category === "courses");
  const methodology = selected.find((item) => item.category === "methodology");
  const contact = selected.find((item) => item.category === "contact");
  const digital = selected.find((item) => item.category === "digital_presence");

  if (String(userLanguage || "pt").startsWith("en")) {
    return [
      `Here is a quick overview of **Talkers Idiomas** based on its official public sources:`,
      "",
      institutional?.summary ? `- ${institutional.summary}` : null,
      courses?.summary ? `- Courses and formats: ${courses.summary}` : null,
      methodology?.summary ? `- Methodology: ${methodology.summary}` : null,
      contact?.summary ? `- Contact: ${contact.summary}` : null,
      digital?.summary ? `- Digital presence: ${digital.summary}` : null,
      "",
      "If you want, I can also detail the methodology, courses, contact channels, or public pages one by one.",
    ].filter(Boolean).join("\n");
  }

  return [
    `Aqui vai um resumo rápido da **Talkers Idiomas** com base nas fontes públicas oficiais:`,
    "",
    institutional?.summary ? `- ${institutional.summary}` : null,
    courses?.summary ? `- Cursos e formatos: ${courses.summary}` : null,
    methodology?.summary ? `- Metodologia: ${methodology.summary}` : null,
    contact?.summary ? `- Contato: ${contact.summary}` : null,
    digital?.summary ? `- Presença digital: ${digital.summary}` : null,
    "",
    "Se você quiser, eu também posso detalhar a metodologia, os cursos, os canais de contato ou as páginas públicas da Talkers.",
  ].filter(Boolean).join("\n");
}
...
async function buildTalkersPublicKnowledgeBundle(query = "", { limit = 4 } = {}) {
  ...
  return {
    text: selected
      ...
      .join("\n\n"),
    direct_answer: buildTalkersDirectAnswer(selected),
