...
function queryLooksCurrent(value = "") {
  return /(hoje|agora|atual|atualizado|ultim|recente|cotacao|cota[cç][aã]o|preco|pre[cç]o|clima|tempo|weather|today|current|latest|news|noticia|noticias)/i.test(
...
function buildExchangeDirectAnswer(payload = {}, userLanguage = "pt") {
  ...
  if (String(userLanguage || "pt").startsWith("en")) {
    return `The latest ${bidLabel} quote is around ${bidValue}${askValue && askValue !== bidValue ? ` to ${askValue}` : ""}. Today's range is ${lowValue} to ${highValue}${updatedAt ? ` (updated ${updatedAt})` : ""}. Example: 1 ${from} ≈ ${bidValue} and 100 ${from} ≈ ${example100}.`;
  }

  return `A cotação mais recente do ${bidLabel} está em torno de ${bidValue}${askValue && askValue !== bidValue ? ` a ${askValue}` : ""}. A faixa do dia está entre ${lowValue} e ${highValue}${updatedAt ? ` (atualizado em ${updatedAt})` : ""}. Exemplo: 1 ${from} ≈ ${bidValue} e 100 ${from} ≈ ${example100}.`;
}
